...
const createMenuCategory = (name: string): MenuItem & {
  ...
  return {
    ...
    print: (indent = "") => {
      console.log(`${indent}═══ ${name.toUpperCase()} ═══`);
      items.forEach(item => item.print(indent));
    },
    isVegetarian: () => items.every(item => item.isVegetarian()),
    add: (item) => items.push(item),
...
<Accordions>
  <Accordion title="✅ Use Composite when...">
  ...
  <Accordion title="❌ Avoid Composite when...">
...
- ✅ Open/Closed: Add new component types without breaking existing code
- ✅ Simplifies client code by treating all elements uniformly
- ✅ Easy to add new leaf or composite types

### Cons
- ❌ Can make design overly general
- ❌ Hard to restrict composite contents to certain types
- ❌ Leaf implementations may have dummy methods for add/remove
