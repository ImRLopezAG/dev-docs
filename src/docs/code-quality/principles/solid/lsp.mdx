...
```typescript
/**
 * ❌ BAD: Square violates LSP because it doesn't behave like a Rectangle
 * 
...
const resizeAndCalculate = (rect: Rectangle): number => {
  ...
  // For a true Rectangle: 5 × 10 = 50
  // But for a Square: 10 × 10 = 100 (because setHeight also set width!)
  return rect.getArea();
};

// Test with Rectangle - works correctly
const rectangle = createRectangle(2, 3);
console.log(resizeAndCalculate(rectangle)); // 50 ✅

// Test with Square - BREAKS!
const square = createSquare(4);
console.log(resizeAndCalculate(square)); // 100 ❌ Expected 50!
...
```typescript
/**
 * ❌ BAD: Penguin violates LSP - not all birds can fly!
 */
...
const migrateBirds = (birds: Bird[]): string[] => {
  return birds.map(bird => bird.fly()); // 💥 Throws for penguin!
};
...
```typescript
/**
 * ✅ GOOD: Proper shape hierarchy that respects LSP
 * 
...
```typescript
/**
 * ✅ GOOD: Proper bird hierarchy with capability-based interfaces
 */
...
// migrateFlyingBirds([penguin]); // ❌ Type error - penguin isn't Flyable
...
```typescript
/**
 * ✅ GOOD: Payment methods with proper LSP compliance
 */
...
// processRefund(crypto, ...); // ❌ Type error - crypto isn't refundable!

// Only card can do recurring
await setupSubscription(creditCard, "monthly");
// setupSubscription(bankTransfer, ...); // ❌ Type error - bank isn't recurring!
```
...
```typescript
/**
 * ✅ GOOD: Storage backends with proper capability interfaces
 */
...
// getFileHistory(memoryStorage, ...); // ❌ Type error!

// Only memory has TTL
await storeWithExpiry(memoryStorage, "cache-item", "data", 3600);
// storeWithExpiry(s3Storage, ...); // ❌ Type error!
```
...
// ❌ BAD: Stricter precondition (only 0-50)
const strictProcessor: BaseProcessor = {
  process(value) {
    if (value > 50) throw new Error("Value too high!"); // 🚨 Violates LSP
    return value * 2;
  },
};

// ✅ GOOD: Same or more lenient precondition
...
// ❌ BAD: Weaker postcondition (might return negative)
const badCalc: Calculator = {
  calculate(a, b) {
    return a - b; // 🚨 Might be negative!
  },
};

// ✅ GOOD: Same or stronger postcondition
...
// ❌ BAD: Would violate history if it mutated existing point
// ✅ GOOD: Returns new point, preserving immutability
```
...
interface Bird extends Animal {
  fly?: () => void; // 🚨 Optional method = potential violation
}

// ❌ Type checking to determine subtype
...
// ❌ Throwing errors for unsupported operations
const square = {
  setWidth(w: number) {
...
|-------------------|-------------------|
| Square IS-A Rectangle | ❌ No (when mutable) |
| Penguin IS-A Bird | ❌ No (if Bird.fly() exists) |
| Stack IS-A List | ❌ No (different operations) |
| Circle IS-A Ellipse | ❌ No (when mutable) |
