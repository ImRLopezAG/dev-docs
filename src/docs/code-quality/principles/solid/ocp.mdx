...
```typescript
/**
 * ❌ BAD: Adding new payment methods requires modifying this function
 * 
...
```typescript
/**
 * ✅ GOOD: Open for extension via new strategy implementations
...
```typescript
/**
 * ✅ GOOD: Extensible discount system
 */
...
```typescript
/**
 * ✅ GOOD: Plugin architecture - ultimate OCP
 */
...
```typescript
/**
 * ✅ GOOD: Decorator pattern - extend behavior without modification
 */
...
const createSmsChannel = (config: {
  ...
  async send(recipientId, message) {
    console.log(`📱 Sending SMS to ${recipientId}: ${message.body.slice(0, 160)}`);
...
const createPushChannel = (config: {
  ...
  async send(recipientId, message) {
    console.log(`🔔 Sending push to ${recipientId}: ${message.title}`);
...
// ❌ Modifying tested code for new features
